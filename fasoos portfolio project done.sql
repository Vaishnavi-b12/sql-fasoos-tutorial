...
2. How many unique customer orders wee made?
Ans: 5
select count(distinct customer_id) from customer_orders;

3. How many successful orders were delivered bby each driver?
     driver_id         Successful orders
	    1                   3
		2                   1
		3                   1
...
4. How mnay of each roll was delivered?
...
7. For each customer,how many delivered rolls had at least1 change and how many had no change?
...
with temp_driver_order(order_id,customer_id,roll_id,not_include_items,extra_items_included,order_date)as
(
...
(
select*,case when not_include_items!='0' and extra_items_included!='0' then 'both inc exc' else'either1 inc or exc' end chg_no_chg
from temp_customer_orders where order_id in (
select order_id from temp_driver_order where new_cancellatio1=0))a 
...
9. What was the total number of rolls were ordered for each hour of the day?
select
...
4. what is the difference between the longest and shortest delivery times for all orders?
...
,cast( case when duration like '%min%' then left(duration, charindex('m',duration)-1) else
 ...
 sdp= total orders sucessfuly deliverd/total orders taken
